    ...
    void Menu_Scene::handle (basics::Event & event)
    {
        if (state == READY)                     // Se descartan los eventos cuando la escena está LOADING
        {
            switch (event.id)
            {
                ...
                case ID(touch-moved):
                {
                    if(menuOpcion==PRINCIPAL){

                        // Si estamos en el menu principal se pueden selecionar opciones
                ...
                case ID(touch-ended):           // El usuario deja de tocar la pantalla
                {
                    if (menuOpcion==PRINCIPAL){
                        ...
                        //si hemos hecho click en help y tocamos la pantalla vuelve al emnu principal
                    else if (menuOpcion == AYUDA)
                    {
                        menuOpcion = PRINCIPAL;
    ...
    void Menu_Scene::update (float time)
    {
        if (!suspended) if (state == LOADING)
        {
            ...
            if (context)
            {
                ...
                if(!(ayudaimg= Texture_2D::create(ID(imagAyud),context, "game-scene/explicacion.png"))
                   ||
                   !(bg= Texture_2D::create(ID(bg),context, "menu-scene/bgMenu.png"))   );
                else {
                    context->add(ayudaimg);
                    context->add(bg);
                }
        ...
        if (!suspended)
        {
            ...
            if (canvas)
            {
                ...
                if (state == READY)
                {
                    ...
                    if (menuOpcion==PRINCIPAL)
                    {
                        canvas->fill_rectangle({0,0},{1280,720},bg.get(),BOTTOM |LEFT );
                        for (auto & option : options)
                        {
                    ...
                    {
                        canvas->fill_rectangle({0,0},{1280,720},ayudaimg.get(),BOTTOM |LEFT );
                    }
